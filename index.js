...
function displayCard(pets, isSortClicked) {
  ...
  if(pets.length == 0){
    ...
    document.getElementById('card-container').classList.add('bg-[#1313130c]', 'w-full', 'rounded-2xl')
    ...
    document.getElementById('card-container').classList.remove('bg-[#1313130c]', 'w-full')

  }

  let mainPetsArray = pets
  if ( isSortClicked ) {
    mainPetsArray = pets.sort(function(a, b){return b.price - a.price});
  }
  
  mainPetsArray.forEach((element) => {
    const cardDiv = document.createElement("div");
    const { image, pet_name, breed, date_of_birth, gender, price, petId } = element;
    ...
    cardDiv.innerHTML = `
        <div class="card bg-base-100 border-[1.3px] border-[#131313]/10 p-4">
            ...
            <div class="flex justify-between">
              <button onclick="loadLikedPetPhoto('${image}')" class="hover:bg-[#0E7A81] hover:text-white  border border-[#0E7A81]/15 md:px-3 md:py-1.5 px-2.5 py-1 rounded-lg">
              ...
              <button id="adoptButton${petId}" onclick="adoptModalButton('adoptButton${petId}')" class="hover:bg-[#0E7A81] hover:text-white text-xs text-[#0E7A81] font-bold border border-[#0E7A81]/15 md:px-4 md:py-1.5 px-2.5 py-1 rounded-lg">Adopt</button>
              <button onclick="loadPetDetailesInfo('${petId}')" class="hover:bg-[#0E7A81] hover:text-white text-xs text-[#0E7A81] font-bold border border-[#0E7A81]/15 md:px-4 md:py-1.5 px-2.5 py-1 rounded-lg">Details</button>
            </div>
...
function adoptModalButton(buttonId){
  ...
  document.getElementById(buttonId).classList.remove('text-[#0E7A81]' ,'hover:bg-[#0E7A81]')
  document.getElementById(buttonId).classList.add('adoptDisabledButton' ,'border-none')
  // console.log(petId);
...
document.getElementById('sortButton').addEventListener('click', (e)=>{
  ...
  setTimeout(()=>{
      catagoryPet.length == 0 ? displayCard(allPets ,true) : displayCard(catagoryPet ,true)
      document.getElementById('spinner').style.display = 'none'
...
function detailesInfoModal(petInfo){
  ...
  const {image, pet_name, breed, date_of_birth, gender, price ,vaccinated_status , pet_details} = petInfo
            ...
            <div>
              ...
              <div class="flex items-center justify-start mb-0.5">
                  ...
                  <p class="text-[13.5px] ml-1.5">Vaccinated status: ${
                    vaccinated_status != null ? vaccinated_status : "Not Available"
                  }</p>
              </div>
            </div>
...
function createButtonDiv(buttonsDiv) {
    buttonsDiv.forEach((element)=>{
        ...
        `
        document.getElementById('catagoryButtonContainer').appendChild(newButtonDiv)
    })
}
